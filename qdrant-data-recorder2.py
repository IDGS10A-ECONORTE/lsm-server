...
class OfflineSignRecorderApp:
    def __init__(self, master):
        self.master = master
        master.title("LSM Data Recorder (Dual Hand)")
        
        # Inicializar MediaPipe y Cámara
        self.hands = hands_config
        ...
        self.difficulty_options = ["FACIL", "MEDIO", "DIFICIL"]
    ...
    # --- LÓGICA DE NORMALIZACIÓN (60D por mano) ---
    def get_normalized_60d_vector(self, landmark_list, handedness_label):
        """
        Calcula el vector normalizado de 60 dimensiones.
        Idéntico al anterior, pero ahora se llamará para cada mano.
        """
        ...
        for i in range(1, 21): 
            ...
            # **NOTA CRÍTICA:** Al grabar, NO aplicamos la reflexión del eje X.
            # Queremos grabar la mano izquierda como izquierda y la derecha como derecha.
            # Esto permitirá al modelo de Qdrant distinguir la posición relativa de ambas manos.
            # La normalización X-mirroring se hará SOLO en el servidor y SOLO para la mano dominante, si es necesario.
            # Para la grabación de 2 manos, guardamos las coordenadas tal cual.
            
            final_vector.extend([x_scaled, y_scaled, z_scaled])

        return final_vector

    # --- NUEVA LÓGICA DE GRABACIÓN DE DOS MANOS ---
    def record_and_save_action(self):
        """
        Ejecuta la acción de guardar y procesa ambas manos.
        """
        ...
        if not self.last_results or not self.last_results.multi_hand_landmarks:
            messagebox.showwarning("Advertencia", "No se detectó ninguna mano. Acércate.")
            self.reset_capture_state()
            return

        # Inicializamos los vectores de mano como un array de ceros (120 dimensiones en total)
        # 60x0 para la mano derecha y 60x0 para la mano izquierda
        right_hand_vector = [0.0] * VECTOR_DIMENSION 
        left_hand_vector = [0.0] * VECTOR_DIMENSION
        hands_detected_count = 0
        
        # Iterar sobre todas las manos detectadas
        for i, hand_landmarks in enumerate(self.last_results.multi_hand_landmarks):
            ...
            if vector_60d and len(vector_60d) == VECTOR_DIMENSION:
                ...
                # Asignar el vector a la mano correspondiente
                if handedness == 'Right':
                    right_hand_vector = vector_60d
                elif handedness == 'Left':
                    left_hand_vector = vector_60d
                # Si se detectan 2 manos, pero ambas son del mismo tipo (ej. Right/Right), solo se usará la última.
                # MediaPipe tiende a ser preciso en esto.
        
        if hands_detected_count == 0:
            messagebox.showwarning("Advertencia", "No se detectó ninguna mano válida.")
            self.reset_capture_state()
            return
            
        # El vector final es la concatenación: [R_x1, R_y1, ..., R_z20, L_x1, L_y1, ..., L_z20] (120D)
        final_vector_120d = right_hand_vector + left_hand_vector

        if len(final_vector_120d) == 2 * VECTOR_DIMENSION:
            data_record = {
                "id": self.point_counter, 
                "sign_name": sign_name,
                "difficulty": difficulty,
                "hands_count": hands_detected_count, # Útil para análisis
                "vector": final_vector_120d,        # VECTOR DE 120 DIMENSIONES
            }
            
            # ... (código para guardar en OUTPUT_FILENAME)
            try:
                with open(OUTPUT_FILENAME, 'a') as f:
                    f.write(json.dumps(data_record) + "\n")
                
                self.point_counter += 1
                self.status_label.config(text=f"✅ '{sign_name}' ({difficulty}) GUARDADA ({hands_detected_count} manos). Puntos: {self.point_counter}", fg="green")
            ...
            self.status_label.config(text="❌ Error de procesamiento del vector (Dimensión incorrecta).", fg="red")
        
        self.reset_capture_state()
        
    # --- Resto del código (countdown, reset_capture_state, update_frame) sigue igual ---

    def start_countdown(self):
        # ... (Función anterior) ...
        # (Sin cambios relevantes en la lógica de lateralidad)
        sign_name = self.sign_name_entry.get().strip().upper()
        if not sign_name:
    ...
    def countdown(self, remaining):
        ...
        if remaining > 0:
            ...
            self.master.after(500, self.record_and_save_action)
            self.master.after(1000, lambda: self.countdown_label.config(text="", fg="orange"))
    ...
    def update_frame(self):
        ...
        if ret:
            frame = cv2.flip(frame, 1) # Espejo visual
            
            rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            rgb_frame.flags.writeable = False
            self.last_results = self.hands.process(rgb_frame) # Procesa hasta 2 manos
